    ...
    <main className="min-h-screen bg-[#f1f5f9] pb-12">
      <header className="w-full border-b border-slate-100 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4 md:px-8">
          <Link href="/menu" aria-label="Voltar ao menu">
            <NovaLogo />
          </Link>
                ...
                <article key={`${item.tipo}-${item.titulo}`} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm md:p-6">
                  ...
                  {item.tipo !== 'livro' && <>
                    <div className="mt-4 space-y-3 text-sm leading-relaxed">
                      <p className="rounded-xl bg-blue-50 p-3 text-slate-700"><strong className="text-[#2c9be3]">O que fazer:</strong> {item.o_que_fazer || item.descricao || `Conheça ${item.titulo} e veja se esta opção combina com o seu momento.`}</p>
                      <p className="rounded-xl bg-slate-50 p-3 text-slate-600"><strong className="text-slate-700">Como avançar:</strong> {item.como_fazer || 'Abra a opção, confira requisitos, custo, modalidade e prazo; depois compare com pelo menos uma alternativa antes de decidir.'}</p>
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                      ...
                      {item.url && <a href={item.url} target="_blank" rel="noreferrer" className="rounded-lg bg-[#2c9be3] px-3 py-2 text-xs font-bold text-white hover:bg-[#2188ca]">Conhecer opção ↗</a>}
                    </div>
                  </>}
                </article>
              ))}
